<template><div><h1 id="语句" tabindex="-1"><a class="header-anchor" href="#语句"><span>语句</span></a></h1>
...
<span class="line">失败</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="跳行标签-标签名" tabindex="-1"><a class="header-anchor" href="#跳行标签-标签名"><span>跳行标签 <code v-pre>:标签名</code></span></a></h2>
...
<ul>
<li>
<p>群消息：<code v-pre>[群]</code> （使得消息只会在&quot;QQ群&quot;内触发）
使用方法：</p>
...
<li>
<p>好友消息：<code v-pre>[好友]</code> （使得消息只会在&quot;好友&quot;聊天才会被触发）
使用方法：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>临时会话消息：<code v-pre>[临时]</code> （使得触发词只会在（群聊-临时会话）触发）<br>
使用方法：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>系统消息：<code v-pre>[系统]</code> （检测系统消息状态，可搭配变量&quot;%Status%&quot;进行识别进群'退群'状态等。）</p>
</li>
<li>
<p>戳一戳消息：<code v-pre>[戳一戳]</code>（检测戳一戳消息，使用此标签请勿使用hd登陆）<br>
相关变量及函数：</p>
<ul>
<li>获取消息: <code v-pre>$获取消息 uin_str1$</code></li>
<li>获取戳人QQ: <code v-pre>$获取消息 uin_str2$</code></li>
<li>获取被戳人QQ: <code v-pre>%Reqid%</code></li>
</ul>
<p>使用方法:</p>
...
<li>
<p>撤回消息：<code v-pre>[撤回]</code> （检测撤回消息，有消息撤回时触发）
使用方法：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>退群消息：<code v-pre>[退群]</code> （检测退群消息）
使用方法：</p>
...
<li>
<p>管理上下变更：<code v-pre>[上下管理]</code> （检测管理员上下变更）
使用方法：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li>
<li>
<p>红包消息：<code v-pre>[红包]</code></p>
</li>
<li>
<p>内部调用：<code v-pre>[内部]</code> （搭配调用&amp;回调使用。指令前加上不可被普通消息触发，仅能被调用或回调）</p>
</li>
<li>
<p>初始化：<code v-pre>[初始化]</code> （初始化所有信息。词库插件每次运行&amp;重连&amp;重启都会进行执行）</p>
</li>
<li>
<p>群初始化：<code v-pre>[群初始化]</code> （初始化所有群，并获取群号信息。词库插件每次运行&amp;重连&amp;重启都会进行执行）</p>
</li>
<li>
<p>总处理：<code v-pre>[总处理]</code> （所有消息都会经过这里进行处理）</p>
<ul>
<li>放至词库文件初始位置既所有消息均会触发<code v-pre>[总处理]</code>进行处理(如放至词库文件初始位置可添加任意判断从而无需在所有指令下重复添加判断)</li>
<li>（用途示例:词库开关，权限判定等）</li>
<li>放至词库文件末尾既除指令外的所有消息均会触发<code v-pre>[总处理]</code>进行处理</li>
<li>约等于一个通配符【但是请不要把通配当总处理使用，会裂开的</li>
<li><code v-pre>[系统]</code>标签的指令无法通过<code v-pre>[总处理]</code>所以请放在<code v-pre>[总处理]</code>指令上方（存疑）</li>
</ul>
<p>使用方法：</p>
